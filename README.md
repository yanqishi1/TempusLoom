...
- **Agent 调色 Chatbox**: 支持在编辑器底部通过单轮 agent 交互生成并应用调色 JSON

## AI Chatbox（第一版）

- 在编辑器底部提供独立的 agent chatbox，可输入自然语言风格描述
- 通过“设置”配置 `Base URL`、`API Key`、模型名称，并选择常见提供商预设
- 当前架构已拆分出独立的 `tempusloom.agent` 模块，便于后续扩展为 skill、MCP、长短期记忆等完整 agent 能力
- 第一版聚焦单次交互：压缩当前预览图、拼接调色 JSON 规则提示词、调用模型、解析 JSON、自动应用到当前图片
